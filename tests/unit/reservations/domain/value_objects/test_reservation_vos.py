...
class TestCheckInOut:
    def test_valid_date_obj(self, validate_vo):
        d = date(2023, 1, 1)
        expected = datetime.combine(d, datetime.min.time(), tzinfo=timezone.utc)
        assert validate_vo(CheckInOut, d) == expected
    ...
    def test_invalid_date_str(self, validate_vo):
        ...
        for d in invalid_dates:
            with pytest.raises(
                ValueError
            ):  # convert_date raises ValueError wrapped in Pydantic validation error?
