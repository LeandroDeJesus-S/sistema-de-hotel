...
class TestInitializeReservationUseCase:
    ...
    def test_success(
        self,
        mocker,  # Added mocker for price_mock
        use_case,
        mock_client_repo,
        mock_room_repo,
        mock_reservation_repo,
    ...
    def test_save_failure(
        ...
        checkout = checkin + timedelta(days=3)
        input_dto = CreateReservationInput(
            client_id=1, room_pk=1, check_in=checkin, check_out=checkout, observations=''
        )
    ...
    def test_overlap_repo_error(
        ...
        mock_client_repo.get_by_id.return_value = Result.Ok(client_entity)
        mock_room_repo.find_by_id.return_value = Result.Ok(room_entity)
        mock_reservation_repo.has_overlapping_reservation.return_value = Result.Err(
            'Repo Error'
        )
    ...
    def test_overlap_message_error(
        self,
        use_case,
        mock_client_repo,
        mock_room_repo,
    ...
    def test_create_entity_failure(
        self,
        use_case,
        mock_client_repo,
        mock_room_repo,
