...
@pytest.mark.django_db
def test_scheduling_created_if_form_is_valid(mocker, client_logged_in, client_fixture, active_room_fixture, schedule_form_data):
    """
    ...
    mocker.patch('schedules.models.Scheduling.full_clean') # Ensure Scheduling.full_clean does not raise exception
    mocker.patch('reservations.models.Reservation.calc_reservation_value', return_value=Decimal('100.00')) # Mock calc_reservation_value
    ...
    assert sch.reservation.checkin == datetime.strptime(schedule_form_data['checkin'], '%Y-%m-%d').date()
    assert sch.reservation.checkout == datetime.strptime(schedule_form_data['checkout'], '%Y-%m-%d').date()
    assert sch.reservation.observations == schedule_form_data['obs']
    assert sch.reservation.client == client_fixture


@pytest.mark.django_db
def test_validation_error_renders_schedule_page_with_message(mocker, client_logged_in, active_room_fixture, schedule_form_data, get_message):
    """
...
@pytest.mark.django_db
def test_operational_error_redirects_to_rooms_with_message(mocker, client_logged_in, active_room_fixture, schedule_form_data, get_message):
    """
...
@pytest.mark.django_db
def test_unexpected_exception_redirects_to_rooms_with_message(mocker, client_logged_in, active_room_fixture, schedule_form_data, get_message):
    """
...
@pytest.mark.django_db
def test_reservation_validation_failure_renders_schedule_page_with_message(mocker, client_logged_in, active_room_fixture, client_fixture, get_message):
    """
...
@pytest.mark.django_db
def test_invalid_captcha_redirects_to_schedule_with_correct_message(mocker, client_logged_in, active_room_fixture, schedule_form_data, get_message):
    """
...
@pytest.mark.django_db
def test_payment_not_processing_renders_page_without_creating_task(client_logged_in, payment_fixture):
    """
