...
def test_scheduling_model_with_valid_data(client_fixture, room_fixture):
    ...
    occupied_reservation = G(Reservation, client=client_fixture, room=room_fixture, checkin=occupied_checkin, checkout=occupied_checkout, status='A', active=True)
    ...
    new_reservation = G(Reservation, client=client_fixture, room=room_fixture, checkin=new_checkin, checkout=new_checkout, status='I')
...
@pytest.mark.parametrize(
    ...
    [
        (
            "room_not_occupied",
            "reservation",
            lambda client, room: G(Reservation, client=client, room=room, checkin=date.today() + timedelta(days=1), checkout=date.today() + timedelta(days=3), status='I')
        ),
        (
            "date_overlap",
            "reservation",
            lambda client, room: [
                G(Reservation, client=client, room=room, checkin=date.today() + timedelta(days=5), checkout=date.today() + timedelta(days=10), status='A', active=True),
                G(Reservation, client=client, room=room, checkin=date.today() + timedelta(days=7), checkout=date.today() + timedelta(days=12), status='I')
            ]
        ),
    ],
    ids=["room_not_occupied", "date_overlap"]
)
def test_scheduling_model_with_invalid_data(client_fixture, room_fixture, scenario, expected_error_message_key, setup_reservations_func):
    """
    ...
    if scenario == "room_not_occupied":
        ...
        existing_reservation, new_reservation = setup_reservations_func(client_fixture, room_fixture)
        scheduling = Scheduling(client=client_fixture, reservation=new_reservation)
        # The exact message depends on the dates, so we'll check for the key and part of the message
        expected_message = ReserveErrorMessages.UNAVAILABLE_DATE.split('{dates}')[0] # Get the part before {dates}
