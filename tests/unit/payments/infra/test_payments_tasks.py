...
class TestSendPaymentConfirmationTask:
    def test_send_payment_confirmation_success(self, mocker, mock_payments_repository, payments_container):
    ...
    def test_send_payment_confirmation_not_found(self, mocker, mock_payments_repository, payments_container):
    ...
    def test_send_payment_confirmation_usecase_error(self, mocker, mock_payments_repository, payments_container):
        mock_usecase = mocker.Mock()
...
class TestProcessRefundTask:
    def test_process_refund_success(self, mocker, mock_payments_repository, payments_container):
        ...
        with payments_container.payment_repo.override(mock_payments_repository), \
            ...
            mock_stripe.process_refund.assert_called_once_with("pi_123", 1000, 'requested_by_customer')

    def test_process_refund_payment_not_found(self, mocker, mock_payments_repository, payments_container):
        mock_payments_repository.get_by_id.return_value = Result.Err("Not found")
    ...
    def test_process_refund_no_pi_id(self, mocker, mock_payments_repository, payments_container):
        payment = mocker.Mock()
        ...
        with payments_container.payment_repo.override(mock_payments_repository):
            result = process_refund(payment_id=1, refund_amount_cents=1000)
            assert result.is_err()
            assert "no associated payment intent ID" in result.unwrap_err().msg

    def test_process_refund_stripe_error(self, mocker, mock_payments_repository, payments_container):
        mock_stripe = mocker.Mock()
        ...
        with payments_container.payment_repo.override(mock_payments_repository), \
            ...
            result = process_refund(payment_id=1, refund_amount_cents=1000)
            assert result.is_err()
            assert "Failed to process refund with Stripe" in result.unwrap_err().msg

    def test_process_refund_update_error(self, mocker, mock_payments_repository, payments_container):
        mock_stripe = mocker.Mock()
        mock_stripe.process_refund.return_value = Result.Ok({})
